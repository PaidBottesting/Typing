...
long long totalSendFailures = 0; // New: track send failures
...
void packetSender(int threadId, const std::string& targetIp, int baseTargetPort, int durationSeconds, int packetSize, int numThreads) {
    ...
    // Pre-generate packet once per thread
    ...
    while (keepSending) {
        ...
        // Reduced port randomization range for speed
        int targetPort = baseTargetPort + (rand() % 1000); // 0-999 offset
