...
def buy_key(message):
    ...
    prefix = "RESELLER" if is_reseller else "TRIAL"
    key = f"{prefix}-{user_id}-{int(time.time())}"
    ...
    if is_reseller:
        bot.reply_to(message, f"Key generated for sale: {key}\n"
                              f"Duration: {duration_str}\n"
                              f"Attack Limit: {attack_limit}s\n"
                              f"Cost: {cost} coins\n"
                              "Share this key with a user for them to redeem with /redeem.")
    else:
