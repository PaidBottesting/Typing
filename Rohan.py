    ...
    with data_lock:
        if os.path.exists(DATA_FILE):
            try:
                ...
                print("Corrupted JSON file, initializing new data")
        return {
            'users': {},
            'keys': {},
            'banned': [],
...
# Redeem cooldown
redeem_cooldown = defaultdict(float)
COOLDOWN_SECONDS = 10
...
def run_attack(user_id, ip, port, duration, chat_id):
    try:
        ...
        bot.send_message(chat_id, attack_complete_msg, parse_mode='Markdown')
...
def send_help(message):
    ...
    if user_type == 'admin' or is_owner:
        help_text += (
            ...
            "   Ex: /addtutorial https://youtube.com/example\n"
...
def send_invite(message):
    ...
    if user_id in data['banned']:
        ...
        return
    if user_id not in data['users']:
        bot.reply_to(message, "⚠️ Register First\nUse /start to join the bot.", parse_mode='Markdown')
        return
...
def show_leaderboard(message):
    ...
    if not leaderboard:
        bot.reply_to(message, "🏆 Leaderboard 🏆\nNo inviters yet!", parse_mode='Markdown')
        return
...
def shutdown_bot(message):
    user_id = str(message.from_user.id)
    if user_id != OWNER_ID:
        bot.reply_to(message, "🔒 Permission Denied\nOnly the owner can shut down the bot.", parse_mode='Markdown')
        return
    ...
    bot.reply_to(message, shutdown_msg, parse_mode='Markdown')
    save_data()
    bot.stop_polling()
    sys.exit(0)
...
def my_info(message):
    ...
    bot.reply_to(message, myinfo_msg, parse_mode='Markdown')
...
def check_coins(message):
    ...
    bot.reply_to(message, coins_msg, parse_mode='Markdown')
...
def buy_key(message):
    ...
    if user['coins'] < cost:
        bot.reply_to(message, f"💸 Insufficient Funds\nYou need {cost} coins, but you have {user['coins']}.", parse_mode='Markdown')
        return
    ...
    bot.reply_to(message, buykey_msg, parse_mode='Markdown')
...
def start_attack(message):
    ...
    user = data['users'][user_id]
    if user['plan'] == 'free':
        bot.reply_to(message, "📩 Upgrade Required\nDm @Rohan2349 to buy access.", parse_mode='Markdown')
        return
    if user_id in attacks:
        bot.reply_to(message, "⚠️ Attack in Progress\nUse /stop to halt it first.", parse_mode='Markdown')
        return
    args = message.text.split()
    if len(args) != 4:
        bot.reply_to(message, "📋 Usage\n/attack [IP] [PORT] [TIME]\nEx: /attack 127.0.0.1 8000 60", parse_mode='Markdown')
        return
    ...
    try:
        ...
        bot.reply_to(message, attack_start_msg, parse_mode='Markdown')
        ...
        bot.reply_to(message, "❌ Invalid Input\nPort and time must be integers.", parse_mode='Markdown')
...
def redeem_key(message):
    ...
    if current_time - redeem_cooldown[user_id] < COOLDOWN_SECONDS:
        bot.reply_to(message, f"⏳ Slow Down\nPlease wait {COOLDOWN_SECONDS} seconds between redeem attempts.", parse_mode='Markdown')
        return
    redeem_cooldown[user_id] = current_time
    ...
    args = message.text.split()
    if len(args) != 2:
        bot.reply_to(message, "📋 Usage\n/redeem [KEY]\nEx: /redeem Rohan1234", parse_mode='Markdown')
        return
    
    key = args[1]
    
    try:
        # Validate key existence and state
        if key not in data['keys']:
            bot.reply_to(message, "❌ Invalid Key\nKey does not exist.", parse_mode='Markdown')
            return
        if not data['keys'][key]['active']:
            bot.reply_to(message, "❌ Invalid Key\nKey is inactive.", parse_mode='Markdown')
            return
        if data['keys'][key]['uses'] <= 0:
            bot.reply_to(message, "❌ Invalid Key\nKey has no remaining uses.", parse_mode='Markdown')
            return
        
        # Validate key attributes
        key_data = data['keys'][key]
        if not isinstance(key_data.get('duration', 0), (int, float)) or key_data['duration'] < 0:
            bot.reply_to(message, "❌ Invalid Key\nKey has invalid duration.", parse_mode='Markdown')
            return
        if 'attack_limit' in key_data and (not isinstance(key_data['attack_limit'], int) or key_data['attack_limit'] < 0):
            bot.reply_to(message, "❌ Invalid Key\nKey has invalid attack limit.", parse_mode='Markdown')
            return
        
        # Get key data
        limit = key_data.get('attack_limit', 600)
        
        # Ensure user exists in data['users']
        if user_id not in data['users']:
            data['users'][user_id] = {
                'type': 'user',
                'plan': 'free',
        ...
        # Update user data
        current_user = data['users'][user_id]
        data['users'][user_id] = {
            'type': 'user',
            'plan': 'premium',
            'limit': limit,
            ...
            'invited_by': current_user.get('invited_by', None),
            'group_invites': current_user.get('group_invites', 0)
        }
        ...
        try:
            ...
            bot.send_message(chat_id, "❌ Error: Failed to save data. Please try again later.", parse_mode='Markdown')
            print(f"Save data error during redeem: {e}")
            return
        ...
        bot.reply_to(message, redeem_msg, parse_mode='Markdown')
    
    except Exception as e:
        bot.send_message(chat_id, "❌ Error: Something went wrong while redeeming the key. Please try again.", parse_mode='Markdown')
        print(f"Error in redeem_key for user {user_id}, key {key}: {e}")
        try:
            save_data()
        except:
            print("Failed to save data after error in redeem_key")
...
def show_plans(message):
    ...
    bot.reply_to(message, plans_msg, parse_mode='Markdown')
...
def spend_coins(message):
    ...
    try:
        ...
        bot.reply_to(message, spend_msg, parse_mode='Markdown')
    
    except ValueError:
        bot.reply_to(message, "❌ Invalid Input\nUse a positive integer.", parse_mode='Markdown')
...
def generate_key(message):
    ...
    if not is_admin(user_id):
        bot.reply_to(message, "🔒 Permission Denied\nAdmin only.", parse_mode='Markdown')
        return
    args = message.text.split()
    if len(args) != 4:
        bot.reply_to(message, "📋 Usage\n/gen [KEY] [DURATION] [USES]\nEx: /gen Rohan9999 1d 5", parse_mode='Markdown')
        return
    ...
    bot.reply_to(message, gen_msg, parse_mode='Markdown')
...
def add_user(message):
    ...
    if len(args) < 4 or len(args) > 5:
        bot.reply_to(message, "📋 Usage\n/add [TYPE] [USER_ID] [DURATION] [COINS (optional)]\nEx: /add reseller 123456789 permanent 1000", parse_mode='Markdown')
        return
    ...
    if type_ not in ['user', 'admin', 'reseller']:
        bot.reply_to(message, "❌ Invalid Type\nMust be user, admin, or reseller.", parse_mode='Markdown')
        return
    if initial_coins < 0:
        bot.reply_to(message, "❌ Invalid Coins\nMust be a positive number.", parse_mode='Markdown')
        return
    ...
    bot.reply_to(message, add_msg, parse_mode='Markdown')
...
def list_users(message):
    ...
    if not data['users']:
        bot.reply_to(message, "👥 User List 👥\nNo users registered yet.", parse_mode='Markdown')
        return
    ...
    users_msg += "-----------------------------------\n📲 Powered by @Rohan2349"
    bot.reply_to(message, users_msg, parse_mode='Markdown')
...
def set_limit(message):
    ...
    args = message.text.split()
    if len(args) != 3:
        bot.reply_to(message, "📋 Usage\n/limit [USER_ID] [SECONDS]\nEx: /limit 123456789 600", parse_mode='Markdown')
        return
    ...
    try:
        limit = int(limit)
        if limit < 0:
            bot.reply_to(message, "❌ Invalid Limit\nMust be non-negative.", parse_mode='Markdown')
            return
        if target_user_id not in data['users']:
            bot.reply_to(message, f"❌ User Not Found\nID {target_user_id} is not registered.", parse_mode='Markdown')
            return
        ...
        bot.reply_to(message, "❌ Invalid Input\nLimit must be a number.", parse_mode='Markdown')
...
def ban_user(message):
    ...
    if target_user_id in data['banned']:
        bot.reply_to(message, f"⚠️ Already Banned\nUser {target_user_id} is already banned.", parse_mode='Markdown')
        return
...
def unban_user(message):
    ...
    args = message.text.split()
    if len(args) != 2:
        bot.reply_to(message, "📋 Usage\n/unban [USER_ID]\nEx: /unban 123456789", parse_mode='Markdown')
        return
    
    target_user_id = args[1]
    if target_user_id not in data['banned']:
        bot.reply_to(message, f"⚠️ Not Banned\nUser {target_user_id} is not banned.", parse_mode='Markdown')
        return
    ...
    bot.reply_to(message, unban_msg, parse_mode='Markdown')
...
def list_keys(message):
    ...
    if not data['keys']:
        bot.reply_to(message, "🔑 Key List 🔑\nNo keys generated yet.", parse_mode='Markdown')
        return
    ...
    keys_msg += "-----------------------------------\n📲 Powered by @Rohan2349"
    bot.reply_to(message, keys_msg, parse_mode='Markdown')
...
def block_key(message):
    ...
    args = message.text.split()
    if len(args) != 2:
        bot.reply_to(message, "📋 Usage\n/block [KEY]\nEx: /block Rohan1234", parse_mode='Markdown')
        return
    
    key = args[1]
    if key not in data['keys']:
        bot.reply_to(message, f"❌ Key Not Found\nKey {key} does not exist.", parse_mode='Markdown')
        return
    if not data['keys'][key]['active']:
        bot.reply_to(message, f"�Completed Successfully!", parse_mode='Markdown')
        return
    ...
    bot.reply_to(message, block_msg, parse_mode='Markdown')
...
def remove_user(message):
    ...
    args = message.text.split()
    if len(args) != 2:
        bot.reply_to(message, "📋 Usage\n/remove [USER_ID]\nEx: /remove 123456789", parse_mode='Markdown')
        return
    ...
    if target_user_id == OWNER_ID:
        bot.reply_to(message, "⚠️ Cannot Remove\nThe owner cannot be removed.", parse_mode='Markdown')
        return
    ...
    bot.reply_to(message, remove_msg, parse_mode='Markdown')
...
def broadcast_message(message):
    ...
    if len(args) < 2:
        bot.reply_to(message, "📋 Usage\n/broadcast [MESSAGE]\nEx: /broadcast Hello everyone!", parse_mode='Markdown')
        return
    ...
    for target_user_id in data['users'].keys():
        try:
            ...
            bot.send_message(target_user_id, broadcast_msg, parse_mode='Markdown')
            success_count += 1
        except Exception as e:
            fail_count += 1
    ...
    bot.reply_to(message, broadcast_result_msg, parse_mode='Markdown')
...
def bot_update(message):
    ...
    bot.reply_to(message, update_msg, parse_mode='Markdown')
...
def terminal_command(message):
    ...
    if len(args) < 2:
        bot.reply_to(message, "📋 Usage\n/terminal [SUBCOMMAND] [ARGS]\nEx: /terminal set thread 1500\nSubcommands: ls, cd, set thread, set packet size", parse_mode='Markdown')
        return
    ...
    if subcommand == 'ls':
        if len(args) < 3:
            bot.reply_to(message, "📋 Usage\n/terminal ls [users|keys|attacks]", parse_mode='Markdown')
            return
        target = args[2].lower()
        if target == 'users':
            if not data['users']:
            ...
            if not attacks:
                bot.reply_to(message, "💥 Attacks\nNo active attacks.", parse_mode='Markdown')
                return
            ls_msg = "💥 Active Attacks 💥\n-----------------------------------\n" + "\n".join([f"{uid}" for uid in attacks.keys()])
        else:
            bot.reply_to(message, "❌ Invalid Target\nUse: users, keys, attacks", parse_mode='Markdown')
            return
        ls_msg += "\n-----------------------------------\n📲 Powered by @Rohan2349"
        bot.reply_to(message, ls_msg, parse_mode='Markdown')
    
    elif subcommand == 'cd':
        bot.reply_to(message, "⚠️ Not Applicable\nUse ls to list data.", parse_mode='Markdown')
    
    elif subcommand == 'set':
        if len(args) < 3:
            bot.reply_to(message, "📋 Usage\n/terminal set [thread|packet size] [VALUE]", parse_mode='Markdown')
            return
        setting = args[2].split(maxsplit=1)
        if len(setting) < 2:
            bot.reply_to(message, "❌ Missing Value\nSpecify a value for the setting.", parse_mode='Markdown')
            return
        ...
        try:
            value = int(value)
            if value <= 0:
                bot.reply_to(message, "❌ Invalid Value\nMust be positive.", parse_mode='Markdown')
                return
            ...
            if key == 'thread':
                ...
                bot.reply_to(message, "❌ Invalid Setting\nUse: thread, packet size", parse_mode='Markdown')
                return
            add_log(user_id, f"Set {key} to {value}")
            bot.reply_to(message, set_msg, parse_mode='Markdown')
        except ValueError:
            bot.reply_to(message, "❌ Invalid Input\nValue must be a positive integer.", parse_mode='Markdown')
    
    else:
        bot.reply_to(message, "❌ Unknown Subcommand\nUse: ls, cd, set thread, set packet size", parse_mode='Markdown')
...
def send_tutorial(message):
    ...
    if not data['tutorials']:
        bot.reply_to(message, "🎥 Tutorial Videos 🎥\nNo tutorials available yet. Check back later!", parse_mode='Markdown')
        return
    ...
    for i, link in enumerate(data['tutorials'], 1):
        tutorial_msg += f"{i}. {link}\n"
    tutorial_msg += "-----------------------------------\n📲 Powered by @Rohan2349"
    bot.reply_to(message, tutorial_msg, parse_mode='Markdown')
...
def add_tutorial(message):
    ...
    if message.chat.type not in ['group', 'supergroup']:
        bot.reply_to(message, "⚠️ Group Only\nThis command must be used in a group.", parse_mode='Markdown')
        return
            ...
            if replied.text and (replied.text.startswith('http://') or replied.text.startswith('https://')):
                ...
                for entity in replied.entities:
                    if entity.type == 'url':
                        ...
                        break
        if not video_link:
            bot.reply_to(message, "📋 Usage\n/addtutorial [VIDEO_LINK]\nEx: /addtutorial https://www.youtube.com/example\nOr reply to a video link in the group.", parse_mode='Markdown')
            return
    ...
    if video_link not in data['tutorials']:
        ...
        bot.reply_to(message, addtutorial_msg, parse_mode='Markdown')
        ...
        bot.reply_to(message, "⚠️ Duplicate\nThis video is already in the tutorial list.", parse_mode='Markdown')
...
def show_logs(message):
    ...
    if not data['logs']:
        bot.reply_to(message, "📜 Activity Logs 📜\nNo logs available yet.", parse_mode='Markdown')
        return
    ...
    bot.reply_to(message, logs_msg, parse_mode='Markdown')
...
def clear_logs(message):
    ...
    bot.reply_to(message, clearlogs_msg, parse_mode='Markdown')
