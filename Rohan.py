...
def send_help(message):
    ...
    if user_type == 'reseller':
        help_text += (
            ...
            "   _Ex: /buykey 1d 300 (100-600s limit)_\n"
            "------------------------------------------------\n\n"
        )
...
def buy_key(message):
    ...
    if is_reseller:
        ...
        buykey_msg += "✅ Use `/redeem {key_code}` to activate!"
...
def redeem_key(message):
    ...
    args = message.text.split()
    if len(args) != 2:
        bot.reply_to(message, "📋 *Usage*\n`/redeem [KEY]`\n*Ex:* `/redeem Rohan-1234`", parse_mode='Markdown')
        return
    key = args[1]
    if key not in data['keys'] or not data['keys'][key]['active'] or data['keys'][key]['uses'] <= 0:
        bot.reply_to(message, "❌ *Invalid Key*\nKey is expired or invalid.", parse_mode='Markdown')
        return
    ...
    redeem_msg = (
        "🔑 *Key Redeemed* 🔑\n"
        "-----------------------------------\n"
        f"✅ *Status:* Premium access granted\n"
...
def generate_key(message):
    ...
    args = message.text.split()
    if len(args) != 4:
        bot.reply_to(message, "📋 *Usage*\n`/gen [KEY] [DURATION] [USES]`\n*Ex:* `/gen Rohan-9999 1d 5`", parse_mode='Markdown')
        return
...
def block_key(message):
    ...
    args = message.text.split()
    if len(args) != 2:
        bot.reply_to(message, "📋 *Usage*\n`/block [KEY]`\n*Ex:* `/block Rohan-1234`", parse_mode='Markdown')
        return
    ...
    if not data['keys'][key]['active']:
        bot.reply_to(message, f"⚠️ *Already Blocked*\nKey `{key}` is already inactive.", parse_mode='Markdown')
        return
...
def add_tutorial(message):
    ...
    args = message.text.split(maxsplit=1)
    if len(args) < 2:
        bot.reply_to(message, "📋 *Usage*\n`/addtutorial [VIDEO_LINK]`\n*Ex:* `/addtutorial https://www.youtube.com/example`", parse_mode='Markdown')
        return
