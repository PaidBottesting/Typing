...
# Coin prices for trial keys (fixed: quoted keys)
COIN_PRICES = {
    '1h': 50,
    '1d': 100,
    '3d': 500,
...
def buy_key(message):
    ...
    key_code = f"Rohan{random.randint(1000, 9999)}"  # No hyphens, simpler format
...
def generate_key(message):
    ...
    if not key.startswith("Rohan") or len(key) != 9 or not key[5:].isdigit():
        key = f"Rohan{random.randint(1000, 9999)}"  # Simplified key format
...
# /addtutorial [VIDEO_LINK] (optional)
