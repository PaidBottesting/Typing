...
def run_attack(user_id, ip, port, duration, chat_id):
    try:
        ...
        stdout, stderr = process.communicate(timeout=duration + 10)  # Buffer for completion
...
def start_attack(message):
    ...
    try:
        ...
        attack_thread.daemon = True  # Ensure thread terminates with main program
        attack_thread.start()
...
def redeem_key(message):
    ...
    with data_lock:
        ...
        data['users'][user_id] = {
            ...
            'invited_by': data['users'].get(user_id, {}).get('invited_by', None),
            'group_invites': data['users'].get(user_id, {}).get('group_invites', 0)
        }
